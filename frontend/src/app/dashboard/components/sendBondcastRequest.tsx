...
interface SendBondcastRequestProps {
  friend: Friend;
  onBack: () => void;
...
export default function SendBondcastRequest({ 
  friend, 
  onBack,
  onNavigateToTopics,
  selectedTopic,
  allFriends
}: SendBondcastRequestProps) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [isSending, setIsSending] = useState(false);
  const [message, setMessage] = useState("");
  const [isSuccess, setIsSuccess] = useState<boolean | null>(null);
  const [sendMode, setSendMode] = useState<'single' | 'all' | 'selected'>('single');
  ...
  const handleSendModeChange = (mode: 'single' | 'all' | 'selected') => {
    setSendMode(mode);
    if (mode === 'selected') {
  ...
  const getButtonText = () => {
    if (sendMode === 'single') {
      return `Send to ${friend.firstname}`;
      ...
      <div className="flex flex-col gap-4 mt-[-7px] h-full">
        ...
        <div className="bg-blue-200/50 rounded-lg p-4 border-2 border-blue-400/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-800 text-lg">{friend.firstname} {friend.lastname}</h3>
              <p className="text-blue-600">@{friend.username}</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe what kind of bondcast you'd like them to create..."
                rows={6}
