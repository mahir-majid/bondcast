...
interface YourBondCastProps {
  recordings: Recording[];
  isLoading: boolean;
  onRecordingsUpdate: (recordings: Recording[]) => void;
}

export default function YourBondCast({ recordings, isLoading, onRecordingsUpdate }: YourBondCastProps) {
  const { user } = useAuth();
  ...
  const yourRecordings = recordings
    .filter(recording => user && recording.sender.id === user.id)
  ...
  if (yourRecordings.length === 0) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-200px)]">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No BondCasts Yet</h3>
          <p className="text-gray-700">You haven&apos;t created any BondCasts yet. Start recording to see them here!</p>
        </div>
      </div>
    );
  }
